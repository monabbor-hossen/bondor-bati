...
<div class="space-y-2.5 mb-4 stagger">
    <a href="?url=auth/generateLink" onclick="return false;" id="btn-gen-link"
       class="flex items-center justify-center gap-2 w-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-400
              font-bold py-3 rounded-xl hover:bg-indigo-500/20 transition-all active:scale-[0.97] text-sm">
        <i class="fas fa-link"></i> Generate Staff Magic Link
    </a>
</div>

<!-- Magic Link Modal -->
<div id="magic-modal" class="fixed inset-0 z-[100] flex items-center justify-center px-4" style="display:none;">
    <div class="absolute inset-0 bg-black/70 backdrop-blur-sm" onclick="document.getElementById('magic-modal').style.display='none'"></div>
    <div class="relative bg-card border border-border rounded-2xl p-6 w-full max-w-sm z-10">
        <h3 class="text-center text-sm font-bold mb-4"><i class="fas fa-link text-accent mr-1"></i> Magic Link Generated</h3>
        <textarea id="magic-url" readonly rows="3"
                  class="w-full bg-surface border border-border rounded-xl px-3 py-2 text-xs text-text-primary mb-3 resize-none"></textarea>
        <button onclick="navigator.clipboard.writeText(document.getElementById('magic-url').value); showToast('Copied!','success');"
                class="w-full bg-accent text-white font-bold py-3 rounded-xl text-sm">
...
document.getElementById('btn-gen-link')?.addEventListener('click', async () => {
    // Prompt for user selection (simplified — in production, use a proper dropdown)
    const userId = prompt('Enter Staff User ID:');
    if (!userId) return;

    const res = await apiPost('?url=auth/generateLink', { user_id: parseInt(userId), hours: 72 });
    if (res.success) {
        document.getElementById('magic-url').value = res.magic_link;
