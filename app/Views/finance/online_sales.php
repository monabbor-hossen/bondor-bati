    ...
    <?php foreach ($balances as $row):
        ...
        <div class="rounded-2xl p-3 border <?= $meta['border'] ?> <?= $meta['bg'] ?> flex flex-col gap-2">
            <div class="flex items-center justify-between">
                <div
                    class="w-7 h-7 rounded-lg <?= $meta['bg'] ?> border <?= $meta['border'] ?> flex items-center justify-center">
                    <i class="fas <?= $meta['icon'] ?> <?= $meta['text'] ?> text-xs"></i>
                </div>
                <?php if ($bal > 0): ?>
                    <span
                        class="text-[0.5rem] font-black uppercase text-emerald-400 bg-emerald-500/10 px-1 py-0.5 rounded-full leading-none">Due</span>
                <?php else: ?>
                    <span
                        class="text-[0.5rem] font-black uppercase text-text-muted bg-surface px-1 py-0.5 rounded-full border border-border leading-none">Clear</span>
                <?php endif; ?>
            </div>
            <div>
                <div class="text-[0.55rem] font-bold text-text-muted uppercase tracking-wide truncate">
                    <?= htmlspecialchars($p) ?>
                </div>
                <div class="text-sm font-black <?= $bal > 0 ? $meta['text'] : 'text-text-muted' ?> leading-tight">
                    ৳<?= number_format(abs($bal), 0) ?></div>
...
<!-- 2. Action Tabs -->
<div class="glass rounded-2xl mb-6 overflow-hidden border border-border/50 animate-slideUp"
    ...
    <div id="panel-sale" class="p-4">
        <form id="saleForm">
            ...
            <div id="sale-edit-banner"
                class="hidden flex items-center justify-between bg-amber-500/10 border border-amber-500/30 rounded-xl px-3 py-2 mb-3">
                <span class="text-xs font-bold text-amber-400"><i class="fas fa-pen mr-1"></i>Editing entry</span>
                <button type="button" onclick="resetSaleForm()"
                    class="text-xs font-bold text-text-muted hover:text-accent"><i
                        class="fas fa-times mr-1"></i>New</button>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div>
                    <label
                        class="block text-[0.65rem] font-bold text-text-muted uppercase tracking-widest mb-1.5">Platform</label>
                    <div class="relative">
                        <select id="sale-platform"
                            class="w-full bg-surface border border-border rounded-xl px-3 py-2.5 text-sm font-semibold text-text-primary appearance-none focus:border-indigo-500 transition-colors pr-7">
                            <option value="Foodpanda">🍔 Foodpanda</option>
                            <option value="Pathao">🏍️ Pathao</option>
                            <option value="Foodi">📦 Foodi</option>
                        </select>
                        <i
                            class="fas fa-chevron-down absolute right-2.5 top-1/2 -translate-y-1/2 text-text-muted text-xs pointer-events-none"></i>
                    </div>
                </div>
                <div>
                    <label
                        class="block text-[0.65rem] font-bold text-text-muted uppercase tracking-widest mb-1.5"><?= __('date') ?></label>
            ...
            <!-- Gross auto-total -->
            <div
                class="flex items-center justify-between bg-surface border border-border/50 rounded-xl px-4 py-2.5 mb-3">
                <span
                    class="text-xs font-bold text-text-muted uppercase tracking-widest"><?= __('gross_sales') ?></span>
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                    <label
                        class="block text-[0.65rem] font-bold text-text-muted uppercase tracking-widest mb-1.5"><?= __('platform_commission') ?></label>
                    <div
                        class="flex bg-surface border border-border rounded-xl overflow-hidden focus-within:border-red-500 transition-colors">
                        ...
                        <select id="sale-commission-type"
                            class="bg-surface border-l border-border px-2 text-xs font-bold text-text-muted focus:outline-none appearance-none cursor-pointer">
                ...
                <div>
                    <label
                        class="block text-[0.65rem] font-bold text-text-muted uppercase tracking-widest mb-1.5"><?= __('discount') ?></label>
                    <div
                        class="flex bg-surface border border-border rounded-xl overflow-hidden focus-within:border-orange-500 transition-colors">
            ...
            <!-- Net Receivable -->
            <div
                class="flex items-center justify-between bg-indigo-500/10 border border-indigo-500/20 rounded-xl px-4 py-3 mb-4">
                <span
                    class="text-xs font-bold text-text-muted uppercase tracking-widest"><?= __('net_receivable') ?></span>
        ...
        <!-- Sales History -->
        <div class="mt-8">
            <h3 class="text-[0.65rem] font-bold text-text-muted uppercase tracking-widest flex items-center gap-2 mb-3">
            ...
            <?php else: ?>
                <div class="space-y-2">
                    <?php foreach ($salesLogs as $log):
                        ...
                        $entryData = ['log_id' => $log['log_id'], 'platform' => $p, 'date' => $log['entry_date'], 'gross' => $log['gross_amount'], 'commission' => $log['commission_amount'], 'discount' => $log['discount_amount'], 'net' => $log['net_amount'], 'items' => $log['items']];
                        $entryJson = htmlspecialchars(json_encode($entryData, JSON_UNESCAPED_UNICODE), ENT_QUOTES);
                        ?>
                        <details id="order-<?= $log['log_id'] ?>"
                            class="group bg-card border border-border/40 rounded-xl overflow-hidden">
                            <summary class="flex items-center gap-2 py-2.5 px-3 cursor-pointer list-none">
                                <!-- Platform icon -->
                                <div
                                    class="w-7 h-7 rounded-lg <?= $meta['bg'] ?> border <?= $meta['border'] ?> flex items-center justify-center flex-shrink-0">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-1 flex-wrap">
                                        <span
                                            class="text-xs font-bold <?= $meta['text'] ?> truncate"><?= htmlspecialchars($p) ?></span>
                                        <span
                                            class="text-[0.5rem] font-semibold uppercase px-1 py-0.5 rounded-full bg-indigo-500/10 text-indigo-400">
                                            <?= __('log_online_sale') ?>
                                        </span>
                                    </div>
                                    <div class="text-[0.55rem] text-text-muted">
                                        <?= $dateLabel ?>         <?= $hasItems ? ' · ' . count($log['items']) . ' items' : '' ?>
                                    </div>
                                </div>

                                <!-- Amount -->
                                <div class="text-right flex-shrink-0">
                                    <div class="text-xs font-black text-text-primary">
                                        ৳<?= number_format((float) $log['net_amount'], 0) ?></div>
                                    <div class="text-[0.5rem] text-text-muted">
                                        Gross ৳<?= number_format((float) $log['gross_amount'], 0) ?>
                                        <?php if ($log['discount_amount'] > 0): ?> · Disc
                                            ৳<?= number_format((float) $log['discount_amount'], 0) ?><?php endif; ?>
                                    </div>
                                </div>

                                <!-- Edit / Delete / Print -->
                                <div class="flex items-center gap-0.5 flex-shrink-0 border-l border-border/40 pl-2 ml-1 print-hide"
                                ...
                                <?php if ($hasItems): ?>
                                    <i
                                        class="fas fa-chevron-down text-text-muted text-[0.6rem] transition-transform duration-200 group-open:rotate-180 flex-shrink-0"></i>
                                <?php endif; ?>
                            </summary>
                            <?php if ($hasItems): ?>
                                    ...
                                    <div class="mt-2 space-y-1">
                                        <?php foreach ($log['items'] as $itm): ?>
                                            <div class="grid grid-cols-12 gap-1 text-xs py-1 border-b border-border/20 last:border-0">
                                                <span
                                                    class="col-span-6 text-text-muted truncate"><?= htmlspecialchars($itm['item_name']) ?></span>
                                                <span class="col-span-2 text-center text-text-muted"><?= (float) $itm['qty'] ?></span>
                                                <span
                                                    class="col-span-2 text-right text-text-muted">৳<?= number_format((float) $itm['unit_price'], 0) ?></span>
                                                <span
                                                    class="col-span-2 text-right font-bold text-text-primary">৳<?= number_format((float) $itm['total_price'], 0) ?></span>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
        ...
        <form id="payoutForm">
            ...
            <div class="mb-4">
                ...
                <div class="relative">
                    ...
                    <i
                        class="fas fa-chevron-down absolute right-3 top-1/2 -translate-y-1/2 text-text-muted text-xs pointer-events-none"></i>
                </div>
                <div id="payout-hint"
                    class="mt-2 text-[0.6rem] font-bold text-text-muted px-1 flex items-center justify-between">
            ...
            <div class="mb-4">
                <label
                    class="block text-[0.65rem] font-bold text-text-muted uppercase tracking-widest mb-1.5"><?= __('payout_amount') ?>
                    (৳)</label>
            ...
            <?php else: ?>
                <div class="space-y-2">
                    <?php foreach ($payoutLogs as $log):
                        ...
                        ?>
                        <div
                            class="bg-card border border-border/40 rounded-xl overflow-hidden flex items-center gap-2 py-2.5 px-3">
                            <!-- Platform icon -->
                            <div
                                class="w-7 h-7 rounded-lg <?= $meta['bg'] ?> border <?= $meta['border'] ?> flex items-center justify-center flex-shrink-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-1 flex-wrap">
                                    <span
                                        class="text-xs font-bold <?= $meta['text'] ?> truncate"><?= htmlspecialchars($p) ?></span>
                                    <span
                                        class="text-[0.5rem] font-semibold uppercase px-1 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400">
                                        <?= __('log_payout') ?>
                                    </span>
                                </div>
                            ...
                            <div class="text-right flex-shrink-0">
                                <div class="text-xs font-black text-emerald-400">
                                    −৳<?= number_format((float) $log['payout_amount'], 0) ?></div>
        ...
        function printOrder(data) {
            ...
            if (data.items && data.items.length) {
                ...
                <div style="display: flex; justify-content: space-between; margin-bottom: 3px;">
                    <div style="flex: 1; text-align: left; padding-right: 5px; line-height: 1.2;">
                        ${itm.item_name} <br> 
                        <span style="font-size: 16px; color: #333;">${parseFloat(itm.qty)} x ৳${parseFloat(itm.unit_price)}</span>
                    </div>
                    <div style="text-align: right; font-weight: bold; font-size: 13px;">৳${parseFloat(itm.total_price)}</div>
                </div>`;
                });
            }
            ...
            if (parseFloat(data.discount) > 0) {
                discHtml = `<div style="display: flex; justify-content: space-between;"><span>Discount:</span><span>-৳${parseFloat(data.discount)}</span></div>`;
            }
            let commHtml = '';
            if (parseFloat(data.commission) > 0) {
                commHtml = `<div style="display: flex; justify-content: space-between;"><span>Commission:</span><span>-৳${parseFloat(data.commission)}</span></div>`;
            }

            printArea.innerHTML = `
            <div style="text-align: center; margin-bottom: 8px;">
                <h2 style="margin: 0; font-size: 18px; font-weight: 900; letter-spacing: 1px;">BONDOR BATI</h2>
                <div style="font-size: 10px; text-transform: uppercase;">Online Delivery Receipt</div>
                <div style="font-size: 14px; margin-top: 5px; font-weight: bold; border-bottom: 1px dashed #000; padding-bottom: 5px; text-transform: uppercase;">${data.platform}</div>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 5px; font-weight: bold;">
                <span>Order #${data.log_id}</span>
                <span>${dateStr}</span>
            </div>
            <div style="border-top: 1px dashed #000; border-bottom: 1px dashed #000; padding: 5px 0; font-size: 12px; margin-bottom: 5px;">
                ${itemsHtml}
            </div>
            <div style="font-size: 11px; margin-bottom: 5px; font-weight: bold;">
                <div style="display: flex; justify-content: space-between;"><span>Gross:</span><span>৳${parseFloat(data.gross)}</span></div>
                ${discHtml}
                ${commHtml}
            </div>
            <div style="border-top: 1px solid #000; padding-top: 5px; font-size: 15px; font-weight: 900; display: flex; justify-content: space-between;">
                <span>NET AMOUNT:</span>
                <span>৳${parseFloat(data.net)}</span>
            </div>
...
<style>
    @media print {
        ...
        #thermal-print-area {
            ...
            top: 0;
            width: 100%;
            /* Printer driver dictates physical width (e.g., 58mm) */
            padding: 5px;
            /* Slight padding for margin */
            background: #fff;
        }
